...
// Export the function for use in your server.js
...
const { NodeIO, BufferUtils, Material } = require('@gltf-transform/core');
...
const cacheFilePath = path.join(__dirname, 'mergeOne.glb');
const googleCacheFilePath = path.join(__dirname, 'googleOne.glb');
const completedCacheFilePath = path.join(__dirname, 'completedCache.glb');
...
async function readGlb() {
    ...
    const buffer1 = await fs1.readFile(cacheFilePath);
    mineDoc = await io.readBinary(new Uint8Array(buffer1));

    const buffer2 = await fs1.readFile(googleCacheFilePath);
    doc = await io.readBinary(new Uint8Array(buffer2));
