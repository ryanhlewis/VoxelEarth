...
static bool read_gltf(const char *filename, TriMesh *mesh);
...
bool TriMesh::read_helper(const char *filename, TriMesh *mesh)
{
	...
	if (c == 'p') {
		...
		eprintf("custom version - Unknown file type.\n");
	}
...
out:
	if (f)
		fclose(f);
	if (!ok) {
		eprintf("custom version - Error reading file [%s].\n", filename);
		return false;
	}
    ...
    for (auto &mesh : model.meshes) {
        ...
        for (auto &primitive : mesh.primitives) {
            ...
            for (auto &image : model.images) {
                ...
                std::cout << "  raw data size: " << image.image.size() << " bytes" << std::endl;
            }
...
bool save_gltf(const char *filename, const TriMesh *mesh) {
    ...
    if (!mesh->normals.empty()) {
        normAccessorIndex = addBufferData(mesh->normals.data(), mesh->normals.size() * sizeof(vec), TINYGLTF_COMPONENT_TYPE_FLOAT, TINYGLTF_TYPE_VEC3, mesh->normals.size(), sizeof(vec));
    }

    // Add colors
    int colorAccessorIndex = -1;
    if (!mesh->colors.empty()) {
        colorAccessorIndex = addBufferData(mesh->colors.data(), mesh->colors.size() * sizeof(Color), TINYGLTF_COMPONENT_TYPE_FLOAT, TINYGLTF_TYPE_VEC3, mesh->colors.size(), sizeof(Color));
    }

    // Add texture coordinates
    int texCoordAccessorIndex = -1;
    if (!mesh->uvs.empty()) {
        texCoordAccessorIndex = addBufferData(mesh->uvs.data(), mesh->uvs.size() * sizeof(UV), TINYGLTF_COMPONENT_TYPE_FLOAT, TINYGLTF_TYPE_VEC2, mesh->uvs.size(), sizeof(UV));
    }
    ...
    int indexAccessorIndex = addBufferData(indices.data(), indices.size() * sizeof(unsigned int), TINYGLTF_COMPONENT_TYPE_UNSIGNED_INT, TINYGLTF_TYPE_SCALAR, indices.size(), 0);

    // Construct primitive
    tinygltf::Primitive primitive;
    primitive.attributes["POSITION"] = posAccessorIndex;
    if (normAccessorIndex != -1) {
    ...
    std::string err;
    std::string warn;
    bool ret = loader.WriteGltfSceneToFile(&model, filename, true, true, true, false);
...
bool read_gltf(const char *filename, TriMesh *mesh) {
    ...
    if (file_ext == "gltf") {
        ...
        std::cerr << "Unsupported file extension: " << file_ext << std::endl;
        return false;
    }
