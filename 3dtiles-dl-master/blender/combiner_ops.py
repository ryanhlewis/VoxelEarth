...
from blender.type_annotations import CombMats
from blender.type_annotations import Diffuse
from blender.type_annotations import MatsUV
from blender.type_annotations import ObMats
from blender.type_annotations import SMCObData
from blender.type_annotations import SMCObDataItem
from blender.type_annotations import Scene
from blender.type_annotations import Structure
from blender.type_annotations import StructureItem
from blender.utils.images import get_image
from blender.utils.images import get_packed_file
from blender.utils.materials import get_diffuse
from blender.utils.materials import get_shader_type
from blender.utils.materials import shader_image_nodes
from blender.utils.materials import sort_materials
from blender.utils.objects import align_uv
from blender.utils.objects import get_polys
from blender.utils.objects import get_uv
from blender.utils.textures import get_texture
...
def set_ob_mode(scn: Scene, data: SMCObData) -> None:
    ob = next(item.ob for item in data if item.type == blender.globs.CL_OBJECT)
    if ob:
...
def get_data(data: Sequence[bpy.types.PropertyGroup]) -> SMCObData:
    ...
    for item in data:
        if item.type == blender.globs.CL_MATERIAL and item.used:
...
def _delete_material(ob: bpy.types.Object, mat_name: str) -> None:
    ...
    if mat_idx > -1:
        if blender.globs.is_blender_2_80_or_newer:
...
def _get_image(mat: bpy.types.Material) -> Union[bpy.types.Image, None]:
    if blender.globs.is_blender_2_79_or_older:
...
def _get_layers(scn: Scene, mats_uv: MatsUV) -> Set[int]:
    return {
        ...
        if item.type == blender.globs.CL_MATERIAL and item.used and item.mat in mats_uv[item.ob.name]
    }
...
def _add_its_from_existing_materials(scn: Scene, existed_ids: Set[int]) -> None:
    ...
    for item in scn.smc_ob_data:
        if item.type != blender.globs.CL_MATERIAL:
            continue
